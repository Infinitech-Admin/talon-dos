...
export default function AdminBottomNav() {
  ...
  return (
    <>
      ...
      {/* Dropup Menu */}
      {activeDropup && (
        <div className="lg:hidden fixed bottom-16 left-0 right-0 bg-white border-t border-slate-200 shadow-2xl z-50 animate-in slide-in-from-bottom duration-300">
          <div className="max-h-[60vh] overflow-y-auto">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 bg-blue-700 text-white">
              ...
              {activeDropup === "More"
                ? // Display More menu items as categories
                  (getDropupItems() as NavItem[]).map((item, index) => {
                    ...
                    const itemActive =
                      isLinkItem && item.path ? isActive(item.path) : false;
                    const groupItemActive =
                      isDropupItem && item.items
                    ...
                    return items.map((item: any, index) => {
                      ...
                      return (
                        <button
                          key={index}
                          onClick={() =>
                            item.path && handleSubItemClick(item.path)
                          }
                          className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-colors ${
                            active
                              ? "bg-blue-50 text-blue-700 font-semibold"
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 z-50 shadow-lg">
        <div className="flex items-center justify-around">
            ...
            return (
              <button
                ...
                className={`flex-1 flex flex-col items-center py-3 transition-colors relative ${
                  active || isDropupOpen ? "text-blue-700" : "text-slate-500"
                }`}
              >
                <div className="relative">
                  <item.icon
                    size={22}
                    className={active || isDropupOpen ? "text-blue-700" : ""}
                  />
                  {item.type === "dropup" && (
                    <ChevronUp
                      size={12}
                ...
                {(active || isDropupOpen) && (
                  <div className="absolute bottom-0 w-10 h-1 bg-blue-700 rounded-t-full" />
                )}
              </button>
            );
          })}
          ...
          <button
            onClick={() =>
              setActiveDropup(activeDropup === "More" ? null : "More")
            }
            className={`flex-1 flex flex-col items-center py-3 transition-colors relative ${
