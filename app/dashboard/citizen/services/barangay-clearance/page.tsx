...
// sessionStorage key used to hold a draft of the form (minus the file
// upload, which isn't JSON-serializable) while the user is sent to
// /login and back.
const DRAFT_KEY = "barangay-clearance-draft";
...
export default function BarangayClearancePage() {
  ...
  // On mount: try to (1) restore a saved draft if the user is coming back
  // from /login, otherwise (2) auto-fill from their profile if they're
  // already logged in. Either way, we DO NOT redirect unauthenticated
  // visitors away — they can browse and fill out the form freely; login
  // is only required at final submit.
  useEffect(() => {
    const init = async () => {
      // 1. Check for a saved draft first (means they just came back from login)
      ...
      try {
        ...
        if (saved) {
          ...
          setFormData((prev) => ({
            ...
            yearsOfResidency: parsed.yearsOfResidency,
          ...
          toast({
            ...
            description:
              "We saved your answers before you logged in. Please re-attach your Valid ID, then continue.",
          });
        }
      } catch (err) {
        console.error("Failed to restore barangay clearance draft:", err);
      }

      // 2. Try to auto-fill from profile if logged in (skip if we just
      // restored a draft, so we don't clobber what the user typed).
      try {
        ...
        // If no user: do nothing — just leave the form blank/editable.
        // Login will be required later, at submit time.
      } catch (error) {
  ...
  const validateStep = () => {
    ...
    if (currentStep === 1) {
      ...
      if (birthDate > minDate) {
        setErrorMessage(
          "Applicant must be at least 18 years old (legal voting age in the Philippines)",
        );
        return false;
      }
      ...
      if (Math.abs(calculatedAge - inputAge) > 1) {
        setErrorMessage(
          "Age does not match birth date. Please check your entries.",
        );
        return false;
      }
  ...
  const saveDraft = () => {
    ...
    try {
      ...
      console.error("Failed to save barangay clearance draft:", err);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    // Application" on the final review step. If not logged in, save
    // everything they've filled out so far, then send them to /login.
    ...
    try {
      ...
      if (response.status === 401) {
        // Session expired mid-flow (e.g. token expired between page load
        // and submit) — same draft-save-and-redirect treatment.
        saveDraft();
        toast({
      ...
      if (response.ok && data.success) {
        ...
        toast({
          title: "Success!",
          description:
            "Your barangay clearance application has been submitted.",
        });
      } else {
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-600">
              Apply for your barangay clearance certificate
            </p>
          </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="space-y-2">
                          ...
                          {formData.birthDate && (
                            <p className="text-xs text-slate-500">
                              Auto-calculated from birth date (must be 18+)
                            </p>
                          )}
                        </div>
                        <div className="space-y-2">
                    ...
                    <div className="space-y-6">
                      ...
                      {formData.validId ? (
                        <div>
                          ...
                          <p className="text-sm">
                            Valid ID: {formData.validId.name}
                          </p>
                        </div>
                      ) : (
                        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                          <p className="text-sm text-yellow-800">
                            ⚠️ No Valid ID attached yet. Go back to Step 3 to
                            upload it before submitting.
                          </p>
                        </div>
                      )}
                    </div>
